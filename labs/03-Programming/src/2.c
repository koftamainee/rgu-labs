...
int creorder_string(const char *str, char **ans) {
    ...
    char *digits = malloc(len + 1), *letters = malloc(len + 1), *others = malloc(len + 1);
    if (!digits || !letters || !others) {
        free(digits); free(letters); free(others);
        return 1;
    }
    ...
    while (*ptr) {
        if (isdigit(*ptr)) digits[digits_cnt++] = *ptr;
        else if (isalpha(*ptr)) letters[letters_cnt++] = *ptr;
        else others[others_cnt++] = *ptr;
        ptr++;
    }
