...
int sums_decomposition(int value, int_vector **result_decompositions,
  ...
  *result_decompositions = (int_vector *)malloc(sizeof(int_vector) * capacity);
  if ((*result_decompositions) == NULL) {
...
int __sums_decomposition_inner(int_vector **result_decompositions,
  ...
  if (remainder == 0) {
    ...
    if (*current_decompositions_capacity < *current_decompositions_count) {
      err = rerealloc((void **)result_decompositions,
                      sizeof(int_vector) * (*current_decompositions_capacity) *
                          GROWTH_FACTOR);
      if (err) {
        return err;
      }
  ...
  for (i = remainder; i > 0; i--) {
    // if vector is empty => summand = 0 (not natural number); else => summand =
    // last vector element
    ...
    if (sumand == 0 ||
      ...
      err = __sums_decomposition_inner(
          result_decompositions, remainder - i, current_decompositions_count,
          current_decompositions_capacity, temp_vector,
