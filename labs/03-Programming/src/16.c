...
int program_03_16(int argc, char* argv[]) {
    ...
    err = permutations(items, ITEMS_COUNT, &result_permutations, &result_permutations_count, compare_int_numbers);
    switch (err) {
        case OK:
            for (i = 0; i < result_permutations_count; ++i) {
                fprintf(fptr, "Permutation №%d: { ", i + 1);
                for (j = 0; j < ITEMS_COUNT; ++j) {
                    fprintf(fptr, "%d%s", result_permutations[i][j], j == ITEMS_COUNT - 1 ? "" : ",");
                }
                fprintf(fptr, " }\n");
...
int permutations(int *items,
    ...
    if (!(target_permutation = (int **)malloc(permutation_count * sizeof(int *)))) {
    ...
    // Recursion
    err = __permutations_inner(items_count,
        current_permutation,
        0,
        not_inserted_items,
        items_count,
...
int __permutations_inner(
    ...
    if (not_inserted_items_count == 0) {
        memcpy(result_permutations[*result_permutations_index_to_insert_permutation], current_permutation, items_count * sizeof(int));
        ++*result_permutations_index_to_insert_permutation;
    ...
    for (not_inserted_item_index = 0; not_inserted_item_index < not_inserted_items_count; ++not_inserted_item_index) {
        ...
        err = __permutations_inner(items_count, current_permutation, current_permutation_to_insert_index + 1, not_inserted_items, not_inserted_items_count - 1, result_permutations, result_permutations_index_to_insert_permutation);
        if (err) {
            printf("WTF");
            return err;
        }
