...
int program_03_16(int argc, char *argv[]) {
    ...
    switch (err) {
        ...
        case RESULT_PERMUTATIONS_INDEX_TO_INSERT_PERMUTATION_NULL:
            printf(
                "Result permutations index to insert permutation ptr is "
                "NULL\n");
            break;
        default:
...
int permutations(int *items, size_t items_count, int ***result_permutations,
    ...
    for (i = 0; i < permutation_count; ++i) {
        // so big so huge
        if (!(target_permutation[i] =
                  (int *)malloc(items_count * sizeof(int)))) {
            for (j = 0; j < i; ++j) {
                free(target_permutation[j]);
            }
            vilka("fff", target_permutation, not_inserted_items,
                  current_permutation);
...
int __permutations_inner(
    ...
    for (not_inserted_item_index = 0;
        ...
        err = __permutations_inner(
            items_count, current_permutation,
            current_permutation_to_insert_index + 1, not_inserted_items,
            not_inserted_items_count - 1, result_permutations,
            result_permutations_index_to_insert_permutation);
        if (err) {
            return err;
        }
