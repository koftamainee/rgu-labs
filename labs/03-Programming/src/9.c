...
int __overvprintf(char **formatted_string, char *restrict _format,
                  va_list valist);
...
int find_zykendorffs_representation(unsigned int num, unsigned int const *row,
                                    size_t numbers_in_row_count,
                                    unsigned int **ans,
...
int __overvprintf(char **formatted_string, char *restrict _format,
                  va_list valist) {
    size_t nums_in_row, nums_in_ans, format_len,
        current_format_index = 0, capacity = BUFSIZ, current_ans_index = 0;
    float float_num;
    double double_num;
    unsigned int unum;
    int err, base, num;
            ...
            switch (*(_format + 1)) {
                ...
                case 'l':
                    if (*(_format + 2) == 'f') {
                        ...
                        memcpy(temp_ans + current_ans_index, s_ans,
                               strlen(s_ans));
                        current_ans_index += strlen(s_ans);
                        free(s_ans);
                        s_ans = NULL;
                ...
                case 'Z':
                    if (*(_format + 2) == 'r') {
                        ...
                        err =
                            generate_fibonacci_row(unum, &u_ptr, &nums_in_row);
                        if (err) {
                            free(u_ptr);
                            return err;
                        }
                        ...
                        err = find_zykendorffs_representation(
                            unum, u_ptr, nums_in_row, &u_ans_ptr, &nums_in_ans);
                        if (err) {
...
int int_to_roman(int num, char **ans) {
    ...
    while (num > 0) {
        while (num >= roman_numerals[i].value) {
            err = rerealloc(
                (void **)&temp,
                (len + strlen(roman_numerals[i].symbol)) * sizeof(char));
            if (err) {
                return err;
            }
            memcpy(temp + len, roman_numerals[i].symbol,
                   strlen(roman_numerals[i].symbol));
