...
int program_03_5(int argc, char* argv[]) {
    ...
    err = is_convex(POLYGON_DOTS_COUNT, &ans, polygon[0], polygon[1], polygon[3], polygon[4]);
    switch (err) {
        case OK:
            if (ans) {
...
int is_convex(int points_count, int *ans, ...) {
    ...
    for (i = 0; i < points_count - 2; ++i) {
        ...
        if (cross != 0) {
            ...
            } else if ((cross > 0 && orientation == -1) || (cross < 0 && orientation == 1)) {
                *ans = 0;
                va_end(valist);
                return OK;
            }
...
void cross_product(dot point_1, dot point_2, dot point_3, double *ans) {
    *ans = (point_2.x - point_1.x) * (point_3.x - point_1.x) - (point_2.y - point_1.y) * (point_3.y - point_1.y);
}